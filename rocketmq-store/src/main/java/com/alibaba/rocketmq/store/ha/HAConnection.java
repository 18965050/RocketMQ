...
/**
 * <pre>
 * master broker对slave broker的HA 连接
 * 只在master broker中运行
 * </pre>
 * 
 * @author lvchenggang
		...
		public void run() {
			...
			while (!this.isStoped()) {
				try {
					...
					if (-1 == this.nextTransferFromWhere) {
						...
						if (0 == HAConnection.this.slaveRequestOffset) {
							...
							masterOffset = masterOffset - (masterOffset % HAConnection.this.haService
									.getDefaultMessageStore().getMessageStoreConfig().getMapedFileSizeCommitLog());
					...
					SelectMapedBufferResult selectResult = HAConnection.this.haService.getDefaultMessageStore()
		...
		private boolean transferData() throws Exception {
			...
			while (this.byteBufferHeader.hasRemaining()) {
				...
				if (writeSize > 0) {
					...
					this.lastWriteTimestamp = HAConnection.this.haService.getDefaultMessageStore().getSystemClock()
							.now();
				} else if (writeSize == 0) {
