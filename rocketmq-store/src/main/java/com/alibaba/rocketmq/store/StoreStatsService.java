...
/**
 * MessageStore的监控服务(比如 put_tps, get_tps等)
 * 
 * @author lvchenggang
 *
 */
public class StoreStatsService extends ServiceThread {
	...
	private static final String[] PutMessageEntireTimeMaxDesc = new String[] { "[<=0ms]", // 0
			"[0~10ms]", // 1
			"[10~100ms]", // 2
			"[100~500ms]", // 3
			"[500ms~1s]", // 4
	...
	private String getGetTransferedTps(int time) {
		...
		try {
			...
			if (this.transferedMsgCountList.size() > time) {
				CallSnapshot lastBefore = this.transferedMsgCountList
						.get(this.transferedMsgCountList.size() - (time + 1));
				result += CallSnapshot.getTPS(lastBefore, last);
			}
	...
	private void sampling() {
		...
		try {
			...
			this.getTimesFoundList
					.add(new CallSnapshot(System.currentTimeMillis(), this.getMessageTimesTotalFound.get()));
			...
			this.transferedMsgCountList
					.add(new CallSnapshot(System.currentTimeMillis(), this.getMessageTransferedMsgCount.get()));
