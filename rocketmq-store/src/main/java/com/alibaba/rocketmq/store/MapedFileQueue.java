 ...
 * <pre>
 * MapedFile队列
 * 每个MapedFile默认大小为1G, 并且长度(long类型)作为文件名
 * </pre>
 * 
 * @author lvchenggang
 *
...
public class MapedFileQueue {
	...
	public void checkSelf() {
		...
		try {
			if (!this.mapedFiles.isEmpty()) {
				...
				int sizeCompute = (int) ((last.getFileFromOffset() - first.getFileFromOffset()) / this.mapedFileSize)
						+ 1;
	...
	public MapedFile getLastMapedFile(final long startOffset, boolean needCreate) {
		...
		if (createOffset != -1 && needCreate) {
			...
			if (this.allocateMapedFileService != null) {
				mapedFile = this.allocateMapedFileService.putRequestAndReturnMapedFile(nextFilePath, nextNextFilePath,
						this.mapedFileSize);
			} else {
				try {
	...
	public boolean resetOffset(long offset) {
		...
		if (!this.mapedFiles.isEmpty()) {
			...
			long lastOffset = mapedFileLast.getFileFromOffset() + mapedFileLast.getWrotePostion();
	...
	public MapedFile findMapedFileByOffset(final long offset, final boolean returnFirstOnNotFound) {
		try {
			...
			if (mapedFile != null) {
				int index = (int) ((offset / this.mapedFileSize)
						- /* 此值应该恒定为0? */(mapedFile.getFileFromOffset() / this.mapedFileSize));
