...
public class IndexService {
	...
	/**
	 * <pre>
	 * 加载${user.home}/store/index/ 目录下的文件
	 * 异常退出情况下, 判断indexFile的endTimeStamp和checkpoint的indexMsgTimestamp
	 * </pre>
	 * 
	 * @param lastExitOK
	 * @return
			...
			if ((keys != null && keys.length() > 0)) {
				...
				for (int i = 0; i < keyset.length; i++) {
					...
					if (key.length() > 0) {
						...
						if (indexFile == null) {
							log.error("putKey error commitlog {} uniqkey {}", req.getCommitLogOffset(),
									req.getUniqKey());
							return;
						}
					}
