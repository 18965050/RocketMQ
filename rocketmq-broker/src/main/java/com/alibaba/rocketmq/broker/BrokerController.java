...
public class BrokerController {
	...
	public boolean initialize() throws CloneNotSupportedException {
		...
		/**
		 * <pre>
		 * 加载${user.home}/store/config/topics.json配置文件到
		 * topicConfigManager的topicConfigTable和dataVersion
		 * </pre>
		 */
		...
		/**
		 * <pre>
		 * 加载${user.home}/store/config/consumerOffset.json配置文件到
		 * consumerOffsetManager的offsetTable
		 * </pre>
		 */
		...
		/**
		 * <pre>
		 * 加载${user.home}/store/config/subscriptionGroup.json配置文件到
		 * subscriptionGroupManager的subscriptionGroupTable
		 * </pre>
		 */
	...
	public void registerProcessor() {
		...
		this.fastRemotingServer.registerProcessor(RequestCode.UNREGISTER_CLIENT, clientProcessor,
				this.clientManageExecutor);
		this.fastRemotingServer.registerProcessor(RequestCode.GET_CONSUMER_LIST_BY_GROUP, clientProcessor,
				this.clientManageExecutor);
		...
		this.fastRemotingServer.registerProcessor(RequestCode.UPDATE_CONSUMER_OFFSET, clientProcessor,
				this.clientManageExecutor);
		this.fastRemotingServer.registerProcessor(RequestCode.QUERY_CONSUMER_OFFSET, clientProcessor,
				this.clientManageExecutor);
		...
		if (this.brokerConfig.isDisableConsumeIfConsumerReadSlowly()) {
			...
			while (it.hasNext()) {
				...
				if (fallBehindBytes > this.brokerConfig.getConsumerFallbehindThreshold()) {
					...
					logProtection.info("[PROTECT_BROKER] the consumer[{}] consume slowly, {} bytes, disable it", group,
							fallBehindBytes);
	...
	public void printWaterMark() {
		logWaterMark.info("[WATERMARK] Send Queue Size: {} SlowTimeMills: {}", this.sendThreadPoolQueue.size(),
				headSlowTimeMills4SendThreadPoolQueue());
		logWaterMark.info("[WATERMARK] Pull Queue Size: {} SlowTimeMills: {}", this.pullThreadPoolQueue.size(),
